<template>
    <div>
        ...
        <el-table border style="width: 100%; margin-top: 10px;" :data="orderNopaymentList.slice((currentPage-1)*pagesize,currentPage*pagesize)" > <!-- 从0开始，每页显示的数据 参考：http://www.manongjc.com/article/22017.html -->
            ...
            <el-table-column label="预订人" prop="o_name" width="90" align="center"> 
            ...
            <el-table-column label="开始停车时间" prop="o_begintime" width="165" align="center">
                <template slot-scope="scope">                    
            ...
            <el-table-column label="结束停车时间" prop="o_endtime" width="165" align="center">
                <template slot-scope="scope">                    
            ...
            <el-table-column label="操作" prop="doit" width="265" align="center">
                <template slot-scope="scope"> 
                    <el-button size="small" type="primary" @click="putbeginorder(scope.row.o_id)" :disabled="scope.row.o_begintime!=='Invalid date'"><i class="el-icon-video-play"></i> 开始计时</el-button>
                    <template v-if="scope.row.o_endtime=='Invalid date'">
                        <el-button size="small" type="warning" @click="putendorder(scope.row.o_id)" :disabled="scope.row.o_begintime=='Invalid date'"><i class="el-icon-video-pause"></i> 结束计时</el-button>
                    </template>
                    <template v-if="scope.row.o_endtime!=='Invalid date'">
                        <el-button size="small" type="success" @click="puttimeorder(scope.row.o_id)" :disabled="scope.row.o_over!==null" ><i class="el-icon-finished"></i> 结算金额</el-button>
                    </template> 
                </template>
            </el-table-column>
...
<script>
    export default {
        ...
        methods: {
            ...
            handleUserList(){
                this.$api.order.getallorderNopayment().then(res =>{
                    if(res.code==1){
            ...
            putbeginorder(val){
                this.$api.order.putorderbegin(val).then(res =>{
                    if(res.code==1){
                        //console.log(res.data)
            ...
            putendorder(val){
                this.$api.order.putorderend(val).then(res =>{
                    if(res.code==1){
                        //console.log(res.data)
                        this.showMessage('success',res.data.message)
                        this.handleUserList()
            ...
            puttimeorder(val){
                this.$api.order.puttime(val).then(res =>{
                    if(res.code==1){
                        //console.log(res.data)
                        this.showMessage('success',res.data.message)
                        this.handleUserList()
