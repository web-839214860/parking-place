<template>
    <div class="allLayer">
        ...
        <el-table style="width: 100%; margin-top: 10px;" :data="carinfoList.slice((currentPage-1)*pagesize,currentPage*pagesize)" > <!-- 从0开始，每页显示的数据 参考：http://www.manongjc.com/article/22017.html -->
        
            <el-table-column label="编号" type="index" width="70" align="center"></el-table-column>

            <el-table-column label="发起人" prop="beginner.b_name" width="110" align="center"></el-table-column>

            <el-table-column label="车位名称" prop="c_name" width="100" align="center"></el-table-column>
            ...
            <el-table-column label="操作" prop="doit" width="355" align="center">
                <template slot-scope="scope"> 
                    <el-button size="small" type="primary" @click="postcars(scope.row.c_id)" :disabled="scope.row.c_status == '发布成功' || scope.row.c_status == '使用中' || scope.row.c_status == '计费中'" ><i class="el-icon-check"></i> 发布</el-button>
                    <el-button size="small" type="primary" @click="putcars(scope.row.c_id)" :disabled="scope.row.c_status == '待发布' || scope.row.c_status == '使用中' || scope.row.c_status == '计费中'" ><i class="el-icon-close"></i> 取消发布</el-button>
        ...
        <el-pagination class="pagination" @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage" :page-sizes="[4, 5, 10, 15]" :page-size="pagesize" layout="total, sizes, prev, pager, next, jumper" :total="carinfoList.length">
        </el-pagination>
...
import { MessageBox } from 'element-ui'
    ...
    export default {
        ...
        methods:{
            ...
            handleUserList(){
                this.$api.car.getallcarsproperty().then(res =>{
                    if(res.code==1){
